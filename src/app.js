...
import { explainApprovalRequirement } from "./domain-js/approvalPolicy.mjs";
...
const storageKey = "openclaw.mission-control.state.v3";
const esc = (value) => String(value ?? "").replace(/[&<>"]/g, (match) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;" }[match]));
const toneFor = { approved: "safe", denied: "danger", "revision-requested": "warning", pending: "warning", complete: "safe", "in-progress": "info", blocked: "danger", available: "safe", planned: "info", gated: "warning", active: "safe", review: "warning", idle: "muted", done: "safe", inbox: "muted", assigned: "info" };
const columns = [
  ["inbox", "Inbox"],
  ["assigned", "Assigned"],
...
function resetState() { state = structuredClone(defaults); saveState("Reset local state", "Restored seeded Mission Control board."); renderShell(); }
function filteredTasks() {
  return state.tasks.filter((task) => task.room === state.selectedRoom && (state.selectedAgent === "all" || task.agent === state.selectedAgent));
}
function currentRoom() { return projectRooms.find((room) => room.id === state.selectedRoom) || projectRooms[0]; }
...
function renderShell() {
  ...
  root.innerHTML = `<div class="ops-shell">
    <header class="ops-topbar">
      ...
      <div class="ops-actions"><button class="icon-button active" title="Agent board">▣</button><button class="icon-button" id="reset-state" title="Reset state">↻</button><span class="time-chip">◷ ${new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span>${pill(projectHealth.overallStatus, "safe")}</div>
    </header>
...
function renderAgentRail(pending) {
  return `<aside class="agent-rail"><div class="rail-header"><span>Agents</span>${pill(`${pending} approvals`, pending ? "warning" : "safe")}</div><button class="all-agents ${state.selectedAgent === "all" ? "active" : ""}" data-agent-filter="all" type="button"><span>All Agents</span><strong>${agents.reduce((sum, agent) => sum + agent.tasks, 0)}</strong></button><div class="agent-cards">${agents.map((agent) => `<button class="agent-card ${state.selectedAgent === agent.name ? "active" : ""}" data-agent-filter="${esc(agent.name)}" type="button"><span class="agent-avatar-xl agent-${agent.tone}">${agent.icon}</span><span><strong>${esc(agent.name)}</strong><small>${esc(agent.role)}</small><em>${esc(agent.platform)} · ${esc(agent.capability)}</em></span><b>${agent.tasks}</b><i class="agent-dot ${agent.status}"></i></button>`).join("")}</div><div class="adapter-mini"><strong>Adapters</strong>${agentPlatforms.map((platform) => `<button data-platform="${platform.id}" type="button"><span>${esc(platform.name)}</span>${pill(platform.status, toneFor[platform.status] || "muted")}</button>`).join("")}</div></aside>`;
}

function renderRoomBoard() {
  const room = currentRoom();
  const tasks = filteredTasks();
  return `<section class="room-stage"><div class="board-actions"><button class="button button--primary" id="new-task" type="button">+ New Task</button><button class="button button--primary" data-command="manage agents" type="button">⚙ Manage Agents</button></div><div class="room-tabs">${projectRooms.map((item) => `<button class="room-tab ${item.id === room.id ? "active" : ""}" data-room="${item.id}" type="button"><strong>${esc(item.name)}</strong><span>${esc(item.owner)}</span></button>`).join("")}</div><div class="room-heading"><div><p class="section-label">${esc(room.owner)} room</p><h1>${esc(room.name)}</h1><span>${esc(room.goal)}</span></div>${pill(`${tasks.length} visible tasks`, "info")}</div><div class="kanban-board">${columns.map(([status, label]) => renderColumn(status, label, tasks)).join("")}</div></section>`;
}
...
function renderDecision(command, persist = true) {
  ...
  if (persist) { state.commandHistory = [{ command, route: decision.route, handler: decision.handler, at: new Date().toLocaleTimeString() }, ...state.commandHistory].slice(0, 30); saveState("Routed command", `${command} -> ${decision.route}/${decision.handler}`); renderShell(); }
}
async function loadDoc(path) {
  state.activeDoc = path;
  try { const response = await fetch(path, { cache: "no-store" }); state.docContent = response.ok ? await response.text() : `Unable to load ${path}: ${response.status}`; }
  catch (error) { state.docContent = `Unable to load ${path}: ${error.message}`; }
  saveState("Loaded Project OS document", path); renderShell();
}
function attachHandlers() {
  document.getElementById("reset-state")?.addEventListener("click", resetState);
  document.getElementById("new-task")?.addEventListener("click", () => { const room = currentRoom(); const id = `task-${Date.now()}`; state.tasks = [{ id, room: room.id, status: "inbox", title: "New mission item", summary: "Operator-created task ready for assignment.", agent: "Product Lead", tags: ["new", "intake"], due: "Next", priority: "medium" }, ...state.tasks]; saveState("New task created", `${room.name}: New mission item`); renderShell(); });
